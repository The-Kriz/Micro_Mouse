...
int targetDistance = 720; // Target distance to rotate motors (in encoder counts)
...
void loop() {
  if (encoderPos1 < targetDistance && encoderPos2 < targetDistance) {
    ...
    analogWrite(PWM1, motorSpeed); // Speed control of Motor A
    analogWrite(PWM2, motorSpeed); // Speed control of Motor B
  } else {
    // Stop both motors
    analogWrite(PWM1, 0);
    analogWrite(PWM2, 0);
